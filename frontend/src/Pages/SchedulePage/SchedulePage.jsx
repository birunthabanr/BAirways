  ...
  const handleBook = async (flight) => {
    try {
      const response = await axios.get(
        `http://localhost:5174/schedule/${flight.FLight_ID}`
      );
      const {
                  ...
                  <tbody>
                    {schedule.map((item, index) => (
                      <tr key={index} className="data">
                        ...
                        <td>{item.Aircraft}</td>
                        <td>
                          {new Date(item.Departure_date_time).toLocaleString()}
                        </td>
                        <td>
                          {new Date(
                            item.Expected_arrival_date_time
                          ).toLocaleString()}
                        </td>
                        ...
                        <td>
                          {item.Modified_BY
                            ? new Date(item.Modified_BY).toLocaleString()
                            : "N/A"}
                        </td>
                        <td>
                          {item.Modified_time
